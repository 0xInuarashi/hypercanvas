...
>
  <svelte:boundary onerror={(e) => console.error(`[Widget] ${node.type} crashed:`, e)}>
    ...
    {#if node.type === 'console'}
      <ConsoleWidget active={!!node.active} defaultCommand={node.label} persistent={!!node.persistent} sessionId={node.sessionId} satellitePassword={node.satellitePassword} fishtankPassword={node.fishtankPassword} nodeId={node.id} onSessionCreated={(sid, restoredSatPwd, restoredFishPwd) => { console.log(`[DBG] CanvasNode onSessionCreated node=${node.id} oldSessionId=${node.sessionId} newSessionId=${sid} restoredSatPwd=${restoredSatPwd ? 'SET' : 'null'} restoredFishPwd=${restoredFishPwd ? 'SET' : 'null'}`); onReplaceNode(node.id, { sessionId: sid, satellitePassword: restoredSatPwd ?? node.satellitePassword, fishtankPassword: restoredFishPwd ?? node.fishtankPassword }) }} onOpenBrowser={onOpenBrowser ? (url) => onOpenBrowser(node.id, url) : undefined} onTextContextMenu={onTextContextMenu ? (text, e) => onTextContextMenu(node.id, text, e) : undefined} />
